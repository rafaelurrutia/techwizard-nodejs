...
<html class="loading">
    ...
    <head>
       ...
       <!--    LIBRESRIAS LOCALES  -->      
        <script src="../js/cpeModel/angular/app.js"></script>
        <script src="../lib/metro/js/metro-dropdown.js"></script>
        <script src="../js/cpeModel/jquery/modalResize.js"></script>
<!--        <script src="../js/cpeModeltest/jquery/validationElementsForm.js"></script> -->
<!--        <script src="../js/cpeModeltest/jquery/keypressed.js"></script> -->
		<script src="../js/cpeModel/jquery/keypress.js"></script>
...
<!--        <script src="../js/default/detectBrowser.js"></script> -->
        <script src="../js/cpeModel/jquery/tableCpeModel.js"></script>
...
*/                                
		...
		var validate = function(option) {
			...
			var data = {
				"name" 			: $("input[name='modalModelCpeInputNameModelCpe']").val().trim(),
				"family" 		: $("select[name='modalCpeModelSelectCpeTypeCpeModel']").val(),	
				"frequency"		: $("select[name='modalCpeModelSelectFrequenceCpeModel']").val(),
				"docsis"		: $("select[name='modalCpeModelSelectDocsisCpeModel']").val(),
			};
			
			if(vacio(data.name)){
				showErrorMessage("input","modalModelCpeInputNameModelCpe",textErrorValidateTitle1,textErrorValidateSubTitle1_1);
				return false;
			}else if(rango(data.name,"3-30")){
				showErrorMessage("input","modalModelCpeInputNameModelCpe",textErrorValidateTitle1,textErrorValidateSubTitle2_1);
				return false;
			}else if(uniqueName(data.name,option)){
				showErrorMessage("input","modalModelCpeInputNameModelCpe",textErrorValidateTitle1,textErrorValidateSubTitle3_1);
				return false;
			}else{
				cleanFormElements("input","modalModelCpeInputNameModelCpe");
			}
			
			if(vacio(data.family)){
				showErrorMessage("select","modalCpeModelSelectCpeTypeCpeModel",textErrorValidateTitle2,textErrorValidateSubTitle1_2);
				return false;
			}else{
				cleanFormElements("select","modalCpeModelSelectCpeTypeCpeModel");
			}
			
			if(vacio(data.frequency)){
				showErrorMessage("select","modalCpeModelSelectFrequenceCpeModel",textErrorValidateTitle3,textErrorValidateSubTitle1_3);
				return false;
			}else{
				cleanFormElements("select","modalCpeModelSelectFrequenceCpeModel");
			}
...
<div id="wrapper">
    ...
    <div id="content">
        ...
        <div id="main">
            ...
            <?php include'cpeModel.php'; ?>            
            <?php 
            
            if($permiso1[2]==0 && $permiso1[3]==0){
				
			}else{
				include'cpeModel_Modal_CpeModel.php'; 
			}       
