...
case 'es':
	...
	nameLowercase= "habitación";
	nameUppercase= "Habitación";
	...
	textErrorValidateTitle1="Error nombre habitación, ";
	...
	datatableTitles = ["Nombre","Opciones"];	
...
case 'en':
	...
	textErrorValidateTitle1="Error room name, ";
	...
	datatableTitles = ["Room name","Options"];
	break;
case 'pt_BR':
	...
	nameLowercase= "quarto";
	nameUppercase= "Quarto";
	...
	textErrorValidateTitle1="Erro nome quarto, ";
...
function findRoom(idRoom,ajax_data) {
	var findRoom = $.getJSON(url.read + idRoom, function(data) {
		if (!data.status) {
			...
			$("#roomTitle").text(' '+textUpdate+' '+nameLowercase);
...
function updateRoom(ajax_data) {
	...
	$.ajax({
		...
		$("button[name='RoomsModalButtonUpdateRoom']").text(textUpdate);
...
function addRoom(ajax_data) {

	var ajax_data_create = {
		"name" 		: ajax_data.name.val().trim(),
	};
	
	$("#roomTitle").text(' '+textAdd+' '+nameLowercase);
	$("button[name='RoomsModalButtonAddRoom']").html('<i class="icon-loading"></i> '+textInsert);
	$("button[name='RoomsModalButtonAddRoom']").text(textInsert);
	...
	$.ajax({
		...
		$("button[name='RoomsModalButtonAddRoom']").text(textAdd);
...
$(document).ready(function() {
	...
	if(getAccess("update")==1){
		roomAccess+='<button class="button mini" id="find"><i class="icon-pencil"></i> '+textUpdate+'</button> ';		
	}
	
	if(getAccess("delete")==1){
		roomAccess+='<button class="button mini" id="delete"><i class="icon-cancel-2"></i> '+textDelete+'</button> ';	
	}
